...
| TM-002 | Contributor changing baseline | Baseline changes are accepted without review | Suppress a real finding through `.reposentinel-baseline.json` | Gate passes while risky content remains | Baseline JSON, credentials | Baseline guide says baselines are reviewed suppressions; changed-file gate rejects root baseline changes and reads the base revision when unchanged | Baseline cannot prove safety; dedicated policy review is still required | Require baseline diff review and explanations for added entries | Alert on baseline file changes in PRs; preserve base-revision extraction | Medium | High | High |
| TM-003 | Contributor changing config | Ignore globs, thresholds, or allowlists are broadened | Exclude sensitive paths from traversal or suppress rules before output | Scanner misses files or findings it would otherwise inspect | Config, repository content | Config is explicit in `.reposentinel.toml`; changed-file gate rejects root config changes before scanning | No policy engine for safe ignore or allowlist patterns | Treat config changes as dedicated security-policy reviews; prefer token-hash or scoped exceptions over broad rule suppressions | Review `.reposentinel.toml` diffs in PRs; keep the normal gate on unchanged base policy | Medium | High | High |
