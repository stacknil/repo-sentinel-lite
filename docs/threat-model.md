...
generic high-entropy strings, missing standard repository files, and
reviewable baseline drift. Its main risk is over-trust: it is not a replacement
for enterprise secret scanning, a clean run does not guarantee that a
repository has no leaked secret or credential, and it does not identify every
credential format.
...
- The scanner does not identify every credential format; it uses suspicious
  filenames and generic entropy heuristics rather than provider-specific
  credential parsers.
...
- Scanner and report formatting: `src/repo_sentinel/scanner.py`
...
- CLI -> repository tree: scanner walks files, ignores configured/default
  paths, reads likely text files, and applies filename and entropy heuristics.
- CLI -> config and baseline: scanner reads `.reposentinel.toml` and baseline
  JSON. Config changes can narrow or widen scan coverage; baseline entries can
...
| `.reposentinel.toml` | Changes scan coverage and thresholds | Integrity |
...
| Repository files | Scanner walks target path | Repo content to scanner | Filename and entropy heuristics are intentionally lightweight | `src/repo_sentinel/scanner.py` |
| `.reposentinel.toml` | Loaded from target root | Repo config to scanner | Ignore globs and thresholds can reduce coverage | `src/repo_sentinel/scanner.py` |
| `.reposentinel-baseline.json` | Auto-loaded from target root | Repo baseline to scanner | Matching findings are suppressed before failure decisions | `src/repo_sentinel/cli.py` |
...
2. Attacker adds broad ignore globs to `.reposentinel.toml`, sensitive paths
   are skipped, and CI still reports success.
...
| TM-003 | Contributor changing config | Ignore globs or thresholds are broadened | Exclude sensitive paths from traversal or reduce detection | Scanner misses files it would otherwise inspect | Config, repository content | Config is explicit in `.reposentinel.toml` | No policy engine for safe ignore patterns | Treat config changes like security changes; keep ignores narrow | Review `.reposentinel.toml` diffs in CI/PRs | Medium | Medium | Medium |
...
| --- | --- | --- |
| `src/repo_sentinel/scanner.py` | Contains traversal, ignore, entropy, redaction, and baseline matching logic | TM-001, TM-003, TM-004 |
| `src/repo_sentinel/cli.py` | Applies baseline suppression before output and failure decisions | TM-002, TM-004 |
| `.pre-commit-hooks.yaml` | Defines provider hook arguments and scan scope for consumers | TM-005 |
| `docs/baseline-review.md` | Sets reviewer expectations for suppression and drift | TM-002, TM-006 |
